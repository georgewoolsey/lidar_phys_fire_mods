...
ladderfuelsr_cbh <- function(
  ...
  if( nrow(lad_profile)-prof_na <= 1 ){
    ...
    lad_profile <- lad_profile %>% 
      dplyr::mutate(lad = dplyr::coalesce(lad, 0.01)) %>% 
...
### Example over mulitiple trees

let's run an example of performing the CBH estimation over multiple trees and combining all of the data
...
trees_cbh <- function(
  ...
  if(
    ...
    stop(paste0(
      "`tree_list` data must contain `treeID` column to estimate CBH."
