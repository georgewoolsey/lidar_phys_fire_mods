...
we'll increase the sample proportion of data to attempt to extract a CBH from the point cloud for this exercise (`tree_sample_prop = 0.9` in `trees_cbh()`) for this exercise. we'll also hold the minimum number of vertical height profile records constant (`min_vhp_n = 2` in `trees_cbh()`). 
...
test_cbh_dta <- 
  1:nrow(dta_temp) %>% 
  purrr::map(\(x) test_cbh_fn(x, my_min_vhp_n = 4, my_tree_sample_prop = 0.9)) %>% 
  dplyr::bind_rows()
```
...
test_cbh_dta_u <- 
  1:nrow(dta_temp) %>% 
  purrr::map(\(x) test_cbh_fn(x, my_min_vhp_n = 4, my_tree_sample_prop = 0.25)) %>% 
  dplyr::bind_rows()
```
