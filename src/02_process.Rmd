...
first, an intermediate function to clip the point cloud to a polygon and run it through the xxx function
...
call_ladderfuelsr_cbh <- function(
      ...
      if(nrow(nlas_one_tree@data)>10){
        ...
        ladderfuelsr_cbh_ans <- ladderfuelsr_cbh(
          ...
          , frst_layer_ht_m = my_frst_layer_ht_m
        ) 
      }else{
...
trees_cbh <- function(
  ...
  cbh_df <- trees_poly %>% 
    ...
    purrr::map(\(x) call_ladderfuelsr_cbh(
      ...
      , my_frst_layer_ht_m = frst_layer_ht_m
    )) %>% 
    dplyr::bind_rows() %>%
    # get rid of treeID
...
trees_cbh_ans %>% sf::st_drop_geometry() %>% 
  dplyr::select(treeID, tree_height_m, tidyselect::ends_with("cbh")) %>% 
  View()
```
