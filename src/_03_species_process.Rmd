...
get_foresttype_raw <- function(savedir=NULL,force=F){
  ...
  # math to set the size parameter
  map_ext <- c(-14469331.213417,2480613.686445,-7114771.213417,6868893.686445)
  (w <- map_ext[3]-map_ext[1])
  (h <- map_ext[4]-map_ext[2])
  ...
  # x combinations
    x_l <- dplyr::tibble(
        xmin = seq(from = map_ext[1], to = map_ext[3], length.out = 5)
      ) %>%
      dplyr::mutate(xmax = dplyr::lead(xmin, n = 1)) %>%
      dplyr::filter(!is.na(xmax))
  # y combinations
    y_l <- dplyr::tibble(
        ymin = seq(from = map_ext[2], to = map_ext[4], length.out = 2)
      ) %>%
  ...
  # chunk_df %>%
  #   dplyr::mutate(
  #     hey=paste(ymax,xmin,xmax,ymin, sep = ",", recycle0 = T)
  #   ) %>%
  #   dplyr::pull(hey)
