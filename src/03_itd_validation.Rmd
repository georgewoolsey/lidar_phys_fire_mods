...
fi_las_to_poly <- function(las,res=0.25,smooth=T) {
  ...
  crowns <- 
    ...
    lidR::filter_poi(!is.na(treeID) & treeID != 0) %>% 
    lidR::crown_metrics(func = .stdtreemetrics, attribute = "treeID", geom = "concave") %>% 
    dplyr::rename(highest_z_val = Z) %>% 
    # sf::st_difference(mask) %>% # to be consistent we
    dplyr::select(treeID,highest_z_val)
  # return
  return(list(
    crowns = crowns
    , mask = mask
...
looks like there is only DBH so we'll use the `highest_z_val` data calculated from the point cloud using our `fi_las_to_poly()` for the matching algorithm above
...
ground_truth_single_tree_match(
  gt_tree = ground_truth_temp$crowns %>% dplyr::arrange(desc(highest_z_val)) %>% dplyr::slice(2)
  ...
  # start with tallest tree and match to get true positives
    for (i in 1:nrow(ground_truth)) {
      match_temp <- ground_truth_single_tree_match(
        ...
        , predictions = predictions %>% dplyr::anti_join(return_df,by="treeID_pred")
        , min_iou_pct = min_iou_pct
      )
      # add to return
      if(!is.null(match_temp)){
        return_df <- return_df %>% 
          dplyr::bind_rows(match_temp %>% dplyr::select(names(return_df)))
      }
      match_temp <- NULL
    }
  # label tps
...
ground_truth_crown_match_ans <- ground_truth_crown_match(
  # ground_truth should be sorted already
  ground_truth = ground_truth_temp$crowns %>% dplyr::arrange(desc(highest_z_val))
  # predictions just needs treeID
  , predictions = ans_crowns_no_mask %>% dplyr::select(treeID)
  , min_iou_pct = 0.5
)
...
agg_ground_truth_crown_match <- function(ground_truth_crown_match_ans) {
  ...
  agg <- ground_truth_crown_match_ans %>% 
    dplyr::count(match_grp) %>% 
    dplyr::mutate(rate=n/sum(n,na.rm = T)) %>% 
    dplyr::mutate(
  ...
  return_df <- dplyr::tibble(match_grp = c("tp","fp","fn")) %>% 
    ...
    dplyr::mutate(dplyr::across(.cols = c(n,rate), .fn = ~dplyr::coalesce(.x,0))) %>% 
    tidyr::pivot_wider(
...
fi_evaluate <- function(
  ...
  ground_truth_crown_match_ans <- ground_truth_crown_match(
    ...
    ground_truth = ground_truth_temp$crowns %>% 
      dplyr::arrange(desc(highest_z_val)) # column calculated in fi_las_to_poly()
    # predictions just needs treeID
    , predictions = ans_crowns_no_mask %>% dplyr::select(treeID)
    , min_iou_pct = min_iou_pct
  )
  ...
  ggplot2::geom_sf(
    data =
      fi_training_submission %>% 
        dplyr::filter(
          plot_name == fi_training_submission$plot_name[9999]
          & ws_fn_nm == fi_training_submission$ws_fn_nm[9999]
        ) %>% 
        dplyr::inner_join(
          fi_evaluate_ans_temp$ground_truth_crown_match %>% 
            dplyr::select(treeID_pred,match_grp)
...
# if(T){
  # filter for only those with this type of validation data
  submission_temp <- fi_training_submission %>% 
    dplyr::inner_join(
      fi_lidar_df %>% 
        sf::st_drop_geometry() %>% 
        dplyr::filter(data_type == "training") %>% 
