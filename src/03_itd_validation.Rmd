...
training_lidar_sum_temp %>% 
  ...
  kableExtra::kbl(
    ...
    , digits = 1
    , col.names = c(
      "site","plot","# points","area m<sup>2</sup>", "points m<sup>-2</sup>"
    )
    , escape = F
  ) %>% 
  kableExtra::kable_styling() %>% 
...
evaluation_img_annttd_crwns %>% 
  ...
  ggplot2::ggplot(ggplot2::aes(x = value, color = name, fill = name)) +
    ...
    ggplot2::scale_x_continuous(breaks = scales::breaks_extended(6), labels = scales::percent) +
...
| Our method     | `r scales::comma(mean(evaluation_img_annttd_crwns$precision),accuracy = 0.01)`| `r scales::comma(mean(evaluation_img_annttd_crwns$recall),accuracy = 0.01)` |[cloud2trees package](https://github.com/georgewoolsey/cloud2trees)|
...
```{r}
p_temp <- evaluation_fld_cllctd_stems %>% 
...
```{r, include=FALSE, eval=TRUE}
ggplot2::ggsave(
  "../data/neon_overall_evaluate_field_stems.jpg"
  , plot = p_temp +
    ggplot2::labs(subtitle = "", x = "recall") +
    ggplot2::theme(
      strip.text = ggplot2::element_blank()
      , plot.title =  ggplot2::element_blank()
      , plot.subtitle =  ggplot2::element_blank()
...
evaluation_fld_cllctd_stems %>% 
  ...
  dplyr::rename_with(.fn = ~stringr::str_remove_all(.x,"recall_")) %>% 
...
| Our method | `r scales::comma(mean(evaluation_fld_cllctd_stems$recall),accuracy = 0.01)` | [cloud2trees package](https://github.com/georgewoolsey/cloud2trees)|
...
p_temp <- evaluation_fld_cllctd_stems %>% 
  ...
  ggplot2::ggplot(ggplot2::aes(x = value, y = siteID, color = name)) +
    ggplot2::geom_jitter(height = 0.12, width = 0, size = 3, alpha = 0.9) +
    ggplot2::geom_boxplot(
      width=0.25,staplewidth=0.4,lwd=0.7
      ,fill=NA,color = "gray9"
      , outliers = F
    ) +
    ggplot2::facet_grid(cols = dplyr::vars(name)) +
...
```{r, include=FALSE, eval=TRUE}
ggplot2::ggsave(
  "../data/neon_plots_evaluate_field_stems.jpg"
  , plot = p_temp +
    ggplot2::labs(subtitle = "", x = "recall") +
    ggplot2::theme(
      strip.text = ggplot2::element_blank()
...
### Summary of evaluation lidar data{#neon_lidar}

Let's get a quick summary of the lidar data we used for evaluation. We expect the evaluation data sets to be sized around 40m x 40m.
...
summary of evaluation lidar data for image annotated crowns ground truth
...
lidar_sum_neon %>% 
  ...
  dplyr::inner_join(lidar_df %>% sf::st_drop_geometry() %>% dplyr::distinct(plot_name,siteID)) %>% 
  dplyr::group_by(siteID) %>% 
  dplyr::summarise(
    dplyr::across(
  ...
  dplyr::mutate(
    value = dplyr::case_when(
      metric=="num_points" ~ scales::comma(value,accuracy=1)
      , metric=="area_m2" ~ scales::comma(value,accuracy=0.1)
      , metric=="points_per_m2" ~ scales::comma(value,accuracy=0.1)
  ...
  dplyr::select(-c(n,min,max,siteID)) %>% 
  dplyr::arrange(unit_lab,metric) %>% 
  kableExtra::kbl(
    caption = "summary of evaluation lidar data for image annotated crowns ground truth"
    , digits = 1
    , col.names = c(
      "site", "Metric"
      , "Mean", "Median"
...
summary of evaluation lidar data for field-collected stems ground truth

```{r}
# we'll save this one for later
lidar_sum_neon <- lidar_sum_neon %>% 
  dplyr::inner_join(
    evaluation_fld_cllctd_stems %>% dplyr::distinct(plot_name)
  ) %>% 
  dplyr::inner_join(lidar_df %>% sf::st_drop_geometry() %>% dplyr::distinct(plot_name,siteID)) %>% 
...
lidar_sum_neon %>% 
  ...
  dplyr::arrange(unit_lab,metric) %>% 
  kableExtra::kbl(
    caption = "summary of evaluation lidar data for field-collected stems ground truth"
    , digits = 1
    , col.names = c(
      "site", "Metric"
      , "Mean", "Median"
...
some of the study sites have very low density point clouds (e.g. <5 points per square meter). let's look at an example low density plot from the DSNY site

```{r, rgl = TRUE, results='asis',fig.show='asis', warning=FALSE}
lidar_df %>% 
  sf::st_drop_geometry() %>% 
  dplyr::filter(
    data_type == "evaluation" &
...
Notice how the canopy is clearly visible even with the low point density, our tree detection method should be able to extract individual overstory trees from these point clouds but defining the crown boundary might be challenging. Tree extraction of suppressed or understory trees from low point density like this is unlikely.
...
Note the predicted crowns classified as commission errors that enclose multiple ground truth tree crowns which are counted as omission errors. Even though the predicted crowns overlap the ground truth crowns, these instances are classified as such due to the criteria that "if the IoU is <0.5: the predicted tree is considered an error" and thus no ground truth instance is matched
...
### Training validation process{#fi_train}
...
fi_training_eval_overall <-
  ...
  dplyr::arrange(desc(f_score),desc(precision),desc(recall),omission_rate,commission_rate) %>% 
  dplyr::mutate(is_best_fn = dplyr::row_number()==1) %>% 
  dplyr::ungroup()
# save it
readr::write_csv(fi_training_eval_overall, "../data/ForInstance_training_eval_overall.csv", append = F, progress = F)
```
...
```{r}
fi_training_eval_overall %>% 
  dplyr::select(ws_fn_nm, omission_rate,commission_rate,recall,precision,f_score) %>% 
  kableExtra::kbl(
    caption = "Best variable window function overall based on training data evaluation"
    , digits = 2
    , col.names = c(
...
fi_training_eval_overall %>% 
  ggplot2::ggplot(ggplot2::aes(x=precision,y=recall,color=ws_fn_nm)) +
  ggplot2::geom_hline(yintercept = median(fi_training_eval_overall$recall),linetype="dashed") +
  ggplot2::geom_vline(xintercept = median(fi_training_eval_overall$precision),linetype="dashed") +
  ggplot2::geom_point(ggplot2::aes(shape=is_best_fn),size = 6) +
  ggplot2::scale_color_manual(values = pal_ws) +
  ggplot2::scale_shape_manual(values = c(20,15)) +
...
# save
readr::write_csv(fi_lidar_df, "../data/ForInstance_lidar_df.csv", append = F, progress = F)
```

```{r, warning=FALSE, message=FALSE, echo=FALSE, include=FALSE}
remove(list = ls()[grep("_temp",ls())])
remove(
  fi_training_submission, fi_training_eval_by_site
  , fi_training_eval_overall
  , training_ground_truth_crown_match_df
  , training_ground_truth_crowns_df
  , training_plot_score_df
...
We can finally process the evaluation data using all that we have gleaned in our [analysis with the training data](#fi_train)

#### Summary of evaluation lidar data

Let's get a quick summary of the lidar data available for evaluation

```{r}
lidar_sum_fi <-
  fi_lidar_df %>% 
  sf::st_drop_geometry() %>% 
  dplyr::filter(data_type == "evaluation") %>% 
  dplyr::pull(f_path) %>% 
  purrr::map(function(x){
    ctg <- lidR::readLAScatalog(x)
    ctg@data %>% 
      ...
      sf::st_drop_geometry() %>% 
      dplyr::mutate(
        fnm = x %>% basename() %>% stringr::str_remove_all("\\.(laz|las)$")    
        , folder = x %>% dirname() %>% stringr::word(-1,sep="/")
        , plot_name = paste0(folder,"_",fnm)
      ) %>% 
      dplyr::relocate(plot_name)
  }) %>% 
  dplyr::bind_rows() %>% 
  dplyr::inner_join(fi_lidar_df %>% dplyr::select(plot_name,siteID,plotID))
# check it
lidar_sum_fi %>% 
  dplyr::arrange(siteID, plotID) %>% 
  dplyr::select(
    siteID, plotID
    , num_points, area_m2, points_per_m2
  ) %>% 
  dplyr::mutate(
    num_points = scales::comma(num_points,accuracy=1)
    , area_m2 = scales::comma(area_m2,accuracy=.1)
    , points_per_m2 = scales::comma(points_per_m2,accuracy=.1)
  ) %>% 
  kableExtra::kbl(
    caption = "FOR-instance summary of evaluation lidar data"
    , digits = 1
    , col.names = c(
      "site","plot","# points","area m<sup>2</sup>", "points m<sup>-2</sup>"
    )
...
Compare these point cloud summaries to the low-density [point clouds in the NEON benchmark](#neon_lidar) from Weinstein et al. ([2021](https://scholar.google.com/scholar?cluster=4986448711981898434&hl=en&as_sdt=0,6)) 

#### Extract trees for evaluation data

as a reminder, here's how many evaluation plots we have by study site
...
fi_lidar_df %>% 
  ...
  dplyr::count(siteID) %>% 
  kableExtra::kbl(
    caption = "FOR-instance sites with conifers and lidar plots for evaluation"
    , col.names = c(
      "site","evaluation (\"test\")<br>plots"
    )
    , escape = F
  ) %>% 
  kableExtra::kable_styling()
...
```{r, results=FALSE, warning=FALSE, message=FALSE}
# where should we save the file?
submission_fn_temp <- "../data/ForInstance_evaluation_submission.gpkg"
# if we don't already have the data, run it
if(!file.exists(submission_fn_temp)){
# if(T){ ### !!!!!!! take out when it gets real
  # just get evaluation
  evaluation_df_temp <- fi_lidar_df %>% 
    dplyr::filter(data_type == "evaluation")
  
  # get trees for each evaluation plot and ws fn combination
  fi_evaluation_submission <- 
    1:nrow(evaluation_df_temp) %>%
    purrr::map(function(x){ 
      # get ws fn
      my_fn_nm <- evaluation_df_temp %>% dplyr::slice(x) %>% dplyr::pull(ws_fn_nm)
  ...
  # generate treeID
  fi_evaluation_submission <- fi_evaluation_submission %>% 
    dplyr::group_by(plot_name) %>% 
    dplyr::mutate(treeID = dplyr::row_number()) %>% 
    dplyr::ungroup()
  
  # save it 
  sf::st_write(fi_evaluation_submission, submission_fn_temp, append = F)
}else{
  fi_evaluation_submission <- sf::st_read(submission_fn_temp, quiet = T)
}
```
...
```{r, warning=FALSE,message=FALSE,results='hide'}
# where should we save the file?
evaluation_eval_temp <- "../data/ForInstance_evaluation_evaluate.csv"
evaluation_eval_m_temp <- "../data/ForInstance_evaluation_evaluate_crown_match.csv"
evaluation_eval_c_temp <- "../data/ForInstance_evaluation_evaluate_gt_crowns.gpkg"
# if we don't already have the data, run it
if(
  !file.exists(evaluation_eval_temp) ||
  !file.exists(evaluation_eval_m_temp) ||
  !file.exists(evaluation_eval_c_temp)
){
# if(T){
  # get unique plot_name,ws_fn_nm
  combs_temp <-
  ...
  # set up empty sets
  evaluation_plot_score_df <- NULL
  evaluation_ground_truth_crown_match_df <- NULL
  evaluation_ground_truth_crowns_df <- NULL
  ...
  for (i in 1:nrow(combs_temp)) {
    ...
    # populate results if available
    if(is.null(fi_evaluate_ans_temp$error)){
      # evaluation_plot_score_df
      evaluation_plot_score_df <- evaluation_plot_score_df %>% 
        dplyr::bind_rows(
          fi_evaluate_ans_temp$result$plot_score %>% 
            dplyr::mutate(
              plot_name = combs_temp$plot_name[i]
      ...
      # evaluation_ground_truth_crown_match_df
      evaluation_ground_truth_crown_match_df <- evaluation_ground_truth_crown_match_df %>% 
        dplyr::bind_rows(
          fi_evaluate_ans_temp$result$ground_truth_crown_match %>% 
            dplyr::mutate(
              plot_name = combs_temp$plot_name[i]
      ...
      # evaluation_ground_truth_crowns_df
      epsg_temp <- fi_evaluate_ans_temp$result$ground_truth_crowns %>% 
        sf::st_crs(parameters=T) %>% 
        purrr::pluck("epsg")
      evaluation_ground_truth_crowns_df <- evaluation_ground_truth_crowns_df %>% 
        dplyr::bind_rows(
          fi_evaluate_ans_temp$result$ground_truth_crowns %>% 
            dplyr::mutate(
              plot_name = combs_temp$plot_name[i]
  ...
  # save
  readr::write_csv(evaluation_plot_score_df, file = evaluation_eval_temp)
  readr::write_csv(evaluation_ground_truth_crown_match_df, file = evaluation_eval_m_temp)
  sf::st_write(evaluation_ground_truth_crowns_df, evaluation_eval_c_temp, append = F)
}else{
  evaluation_plot_score_df <- readr::read_csv(evaluation_eval_temp, progress = F, show_col_types = F)
  evaluation_ground_truth_crown_match_df <- readr::read_csv(evaluation_eval_m_temp, progress = F, show_col_types = F)
  evaluation_ground_truth_crowns_df <- sf::st_read(evaluation_eval_c_temp, quiet = T)
}
```
...
table of results (roughly matches table 6 from [Xiang et al. 2024](https://doi.org/10.1016/j.rse.2024.114078))

```{r}
evaluation_plot_score_df %>% 
  dplyr::inner_join(
    fi_lidar_df %>% 
      dplyr::select(plot_name,siteID,plotID)
    , by = "plot_name"
  ) %>% 
  dplyr::mutate(
    ref_trees = tp_n+fn_n
  ...
  dplyr::mutate(
    ...
    , rmse_height_m = scales::comma(rmse_height_m,accuracy=0.01)
  ) %>% 
  dplyr::arrange(siteID,plotID) %>% 
  kableExtra::kbl(
    caption = "FOR-instance individual tree segmentation results for each plot in the evaluation set"
    , col.names = c(
      "site","plot"
      , "reference<br>trees", "detected<br>trees", "correct<br>trees"
      , "omission<br>rate"
      , "commission<br>rate"
      ,"recall","precision","F-score"
      , "RMSE<br>height (m)"
...
to look at the results in space we'll make each plot separately and then combine via `patchwork`

```{r, results=F,message=FALSE,warning=FALSE}
# list of plots to process
plt_nms_temp <- evaluation_plot_score_df %>% 
  dplyr::inner_join(
    fi_lidar_df %>% 
      dplyr::select(plot_name,siteID,plotID)
    , by = "plot_name"
...
plt_fn <- function(
    rn
    , list_df = plt_nms_temp
    , my_evaluation_ground_truth_crowns_df = evaluation_ground_truth_crowns_df %>% sf::st_set_crs(NA)
    , my_evaluation_ground_truth_crown_match_df = evaluation_ground_truth_crown_match_df
    , my_fi_evaluation_submission = fi_evaluation_submission %>% sf::st_set_crs(NA)
    , my_pal_match_grp = pal_match_grp
    , my_evaluation_plot_score_df = evaluation_plot_score_df
) {
  # evaluation
  e_temp <- my_evaluation_ground_truth_crowns_df %>%
    dplyr::filter(plot_name==list_df$plot_name[rn]) %>%
    dplyr::inner_join(
      my_evaluation_ground_truth_crown_match_df %>%
        dplyr::select(plot_name,treeID,match_grp)
      , by = dplyr::join_by(plot_name,treeID)
    ) %>% 
    dplyr::mutate(match_grp=forcats::fct_rev(match_grp)) %>% 
    dplyr::arrange(desc(match_grp))
  # prediction
  p_temp <- my_fi_evaluation_submission %>%
    dplyr::filter(plot_name==list_df$plot_name[rn]) %>% 
    dplyr::inner_join(
      my_evaluation_ground_truth_crown_match_df %>%
        dplyr::select(plot_name,treeID_pred,match_grp)
      , by = dplyr::join_by(plot_name, treeID==treeID_pred)
    ) %>%
    sf::st_set_crs(sf::st_crs(evaluation_ground_truth_crown_match_df)) %>% 
    dplyr::mutate(match_grp=forcats::fct_rev(match_grp)) %>% 
    dplyr::arrange(desc(match_grp))
  # plot it
  ggplot2::ggplot(mapping = ggplot2::aes(fill = match_grp, color = match_grp)) +
    ggplot2::geom_sf(
      data = e_temp
      , color = "gray" ,alpha=0.7, lwd = 0.1
    ) +
    ggplot2::geom_sf(
      data = p_temp %>% dplyr::filter(match_grp=="commission")
      , alpha = 0
      , lwd = 2.2
    ) +
    ggplot2::geom_sf(
    ...
    ggplot2::coord_sf(default_crs = NA) +
    # ggplot2::facet_wrap(facets = dplyr::vars(plot_name)) +
    ggplot2::scale_fill_manual(values = my_pal_match_grp, name = "") +
    ggplot2::scale_color_manual(values = my_pal_match_grp, name = "") +
    ggplot2::labs(
      subtitle = paste0(
        list_df$siteID[rn]
        , " plot "
    ...
    ggplot2::theme(
      ...
      # , plot.background = element_rect(color = "gray88", fill = NA, linewidth = 0.5)
    ) +
    ggplot2::guides(
      fill = ggplot2::guide_legend(
        override.aes = list(
          color = c(my_pal_match_grp["true positive"],"transparent",my_pal_match_grp["commission"])
          , lwd = 1 # c(1,0.1,1)
        )
      )
      , color = "none"
...
# plt_fn(3)
# map over plots
plt_list <- 1:nrow(plt_nms_temp) %>% 
  purrr::map(plt_fn)
# plt_list[length(plt_list)]
```

combine the figures as tiles

```{r, fig.height=9.5,fig.width=8.5}
p_temp <- patchwork::wrap_plots(plt_list, ncol = 3, guides = "collect") &
...
```{r, include=FALSE, eval=TRUE}
ggplot2::ggsave(
  "../data/forinstance_validation_plots.jpg"
  , plot = p_temp +
    ggplot2::theme(
      strip.text = ggplot2::element_blank()
      # , plot.title =  ggplot2::element_blank()
...
The ground truth trees are shown as filled polygons while our predicted crowns are shown as unfilled polygon outlines. Note the predicted crowns classified as commission errors that enclose multiple ground truth tree crowns which are counted as omission errors. Even though the predicted crowns overlap the ground truth crowns, these instances are classified as such due to the criteria that "if the IoU is <0.5: the predicted tree is considered an error" and thus no ground truth instance is matched.

Let's look at the distribution of tree heights in the evaluation versus the predicted data for *all* trees (true positive, omission, and commission)

```{r}
# scales::show_col(RColorBrewer::brewer.pal(n = 3, name = "Dark2"))
pal_gt_train = c(
  "ground truth"=RColorBrewer::brewer.pal(n = 3, name = "Dark2")[1]
  , "prediction"= RColorBrewer::brewer.pal(n = 3, name = "Dark2")[3]
)
# plot
evaluation_ground_truth_crown_match_df %>% 
  dplyr::inner_join(
  ...
  dplyr::select(siteID,plotID,tidyselect::contains("tree_height")) %>% 
  tidyr::pivot_longer(
    cols = tidyselect::contains("tree_height")
  ) %>% 
  dplyr::arrange(siteID,plotID) %>% 
  dplyr::mutate(
    lab = paste0(
    ...
    , name = dplyr::case_match(
      name
      , "gt_tree_height" ~ "ground truth"
      , "tree_height_m" ~ "prediction"
    )
  ) %>% 
  ggplot2::ggplot(
    mapping = ggplot2::aes(x = value, color = name, fill = name, group = name)
  ) +
  ggplot2::geom_density(alpha = 0.7, lwd = 1) +
  ggplot2::facet_wrap(facets = dplyr::vars(lab), ncol = 3, scales = "free") +
  ggplot2::scale_x_continuous(breaks = scales::breaks_extended(6)) +
  ggplot2::scale_fill_manual(values = pal_gt_train, name = "") +
  ggplot2::scale_color_manual(values = pal_gt_train, name = "") +
  ggplot2::labs(
    x = "height (m)", y = ""
  ) +
  ggplot2::theme_light() +
...
These distributions indicate that, in general, our aerial point cloud tree detection methodology missed lower canopy trees. Under-detection of suppressed or understory trees based on aerial lidar data is a well-known shortcoming of these methodologies.

For the most part, our aerial point cloud tree detection method performed well at identifying tree tops based on the tree height RMSE values. Let's look at the matched tree pairs (true positive) and the predicted height difference.

```{r, fig.height=9.1, fig.width=8.5}
# plot
evaluation_ground_truth_crown_match_df %>% 
  dplyr::inner_join(
    fi_lidar_df %>% 
  ...
  dplyr::mutate(height_diff=abs(height_diff )) %>% 
  dplyr::select(plot_name,treeID,siteID,plotID,tidyselect::contains("tree_height"),height_diff) %>% 
  tidyr::pivot_longer(
    cols = tidyselect::contains("tree_height")
  ) %>% 
  dplyr::group_by(siteID,plotID,treeID) %>% 
  dplyr::mutate(x_ht = max(value)) %>% 
  dplyr::group_by(siteID,plotID) %>% 
  dplyr::mutate(
    rk_ht = dplyr::dense_rank(desc(x_ht))
    , force_lab = rk_ht==max(rk_ht) | rk_ht==min(rk_ht)
  ) %>% 
  dplyr::arrange(siteID,plotID,desc(x_ht) ) %>% 
  dplyr::ungroup() %>% 
  dplyr::inner_join(evaluation_plot_score_df %>% dplyr::select(plot_name, rmse_height_m)) %>% 
  dplyr::mutate(
    lab = paste0(
        siteID
        , " plot "
        , plotID
        , "\nheight RMSE: "
        , scales::comma(rmse_height_m,accuracy=0.01)
        , " (m)"
      )
    , name = dplyr::case_match(
      name
    ...
    , ht_lab = ifelse(
      rk_ht %in% seq(from = 1, by = 3, length.out = 111) | force_lab
      , value
      , NA
    )
  ) %>% 
  ...
  ggplot2::geom_line(mapping = ggplot2::aes(group = treeID, color = height_diff)) +
  ggplot2::scale_colour_gradient(low = "gray66", high = "gray19") +
  ggnewscale::new_scale_color() +
  ggplot2::geom_point(mapping = ggplot2::aes(color = name)) +
  ggplot2::scale_color_manual(values = pal_gt_train, name = "") +
  ggrepel::geom_text_repel(
    mapping = ggplot2::aes(
      x = ifelse(name=="ground truth",name,NA)
      , label = scales::comma(ht_lab,accuracy=0.1)
    )
    , segment.color  = pal_gt_train["ground truth"]
    , size = 2.5, max.overlaps = 3
    , nudge_x = -0.2, direction = "y", hjust = "right"
    , min.segment.length = 0.2
  ) +
  ggrepel::geom_text_repel(
    mapping = ggplot2::aes(
      x = ifelse(name=="prediction",name,NA)
      , label = scales::comma(ht_lab,accuracy=0.1)
    )
    , segment.color  = pal_gt_train["prediction"]
    , size = 2.5, max.overlaps = 3
...
Notice that one tree in a few plots had a significant impact on increasing the plot-level tree height RMSE value. However, overall, our method performed well detecting the tree tops

##### Site-level scores

To aggregate our plot-level scores to the site level we'll summarize our confusion matrix ($TP$, $FP$, $FN$) by site.

Our table of results will align with table 5 from Wielgosz et al. ([2024](https://doi.org/10.1016/j.rse.2024.114367)) for comparison
...
eval_temp <- 
  ...
  dplyr::group_by(siteID) %>% 
  dplyr::summarise(dplyr::across(c(tp_n,fp_n,fn_n), .fn = sum)) %>%
  dplyr::ungroup() %>%
  ...
  dplyr::left_join(
    evaluation_ground_truth_crown_match_df %>% 
      ...
      dplyr::ungroup() %>% 
      dplyr::select(siteID,rmse_height_m)
  ) %>% 
  dplyr::select(
    siteID
    , omission_rate,commission_rate,recall,precision,f_score
...
eval_temp %>% 
  ...
  dplyr::arrange(siteID) %>% 
  kableExtra::kbl(
    caption = "FOR-instance individual tree segmentation results for study site in the evaluation set with conifers"
    , col.names = c(
      "site"
      , "omission<br>rate"
      , "commission<br>rate"
...
here is the comparison to state-of-the-art methods that utilize deep learning models for tree segmentation from point cloud data as reported in Wielgosz et al. ([2024](https://doi.org/10.1016/j.rse.2024.114367))

* Straker, A., Puliti, S., Breidenbach, J., Kleinn, C., Pearse, G., Astrup, R., & Magdon, P. (2023). Instance segmentation of individual tree crowns with YOLOv5: A comparison of approaches using the ForInstance benchmark LiDAR dataset. *ISPRS Open Journal of Photogrammetry and Remote Sensing, 9*, 100045. [https://doi.org/10.1016/j.ophoto.2023.100045](https://doi.org/10.1016/j.ophoto.2023.100045)
* Xiang, B., Wielgosz, M., Kontogianni, T., Peters, T., Puliti, S., Astrup, R., & Schindler, K. (2024). Automated forest inventory: Analysis of high-density airborne LiDAR point clouds with 3D deep learning. *Remote Sensing of Environment, 305*, 114078. [https://doi.org/10.1016/j.rse.2024.114078](https://doi.org/10.1016/j.rse.2024.114078)
* Wielgosz, M., Puliti, S., Xiang, B., Schindler, K., & Astrup, R. (2024). SegmentAnyTree: A sensor and platform agnostic deep learning model for tree segmentation using laser scanning data. *Remote Sensing of Environment, 313*, 114367. [https://doi.org/10.1016/j.rse.2024.114367](https://doi.org/10.1016/j.rse.2024.114367)

```{r}
df_temp <- read.csv(text = "
siteID,study,omission_rate,commission_rate,recall,precision,f_score,rmse_height_m
NIBIO,Straker et al. (2023),0.33,,0.67,,,
NIBIO,Xiang et al. (2024),0.12,0.03,0.88,0.97,0.92,
NIBIO,Wielgosz et al. (2024),0.12,0.09,0.88,0.91,0.88,3.4
CULS,Straker et al. (2023),0,,1,,,
CULS,Xiang et al. (2024),0,0.13,1,0.87,0.93,
CULS,Wielgosz et al. (2024),0,0,1,1,0.99,0.14
SCION,Straker et al. (2023),0.14,,0.86,,,
SCION,Xiang et al. (2024),0.13,0.04,0.87,0.96,0.91,
SCION,Wielgosz et al. (2024),0.08,0.07,0.92,0.93,0.91,1.7
")

df_temp %>% 
  dplyr::bind_rows(
    eval_temp %>% 
    dplyr::mutate(study = "Our method") %>% 
    dplyr::relocate(study, .after = siteID)
  ) %>% 
  dplyr::arrange(siteID) %>% 
  dplyr::mutate(
    dplyr::across(
  ...
  kableExtra::kbl(
    caption = "FOR-instance individual tree segmentation results for existing studies employing deep learning against our results for the tested datasets"
    , col.names = c(
      "site","study"
      , "omission<br>rate"
      , "commission<br>rate"
...
While these deep learning methods offer superior accuracy and detail, capturing complex 3D tree structures often missed by 2D Canopy Height Model (CHM) approaches, their adoption is hindered by substantial computational demands, complex data variability requiring extensive annotated training sets, and the inherent complexity of 3D model design and training. In contrast, 2D CHM methods are computationally efficient and often provide adequate results, making them more attractive for many applications. Consequently, despite the potential of deep learning to enhance forest analysis, the relative simplicity and lower computational cost of 2D CHM methods have contributed to their wider adoption.
...
summarize lidar from both benchmarks for reporting table

```{r}
# site descriptions
df_temp <- read.csv(text = "
siteID,desc,location
CULS,Coniferous dominated temperate forest with productive even-aged Scots pine (<i>Pinus Sylvestris</i>) stands,\"Central Bohemia region, Czech Republic\"
NIBIO,Coniferous dominated boreal forest composed of mature forests dominated by Norway spruce (<i>Picea abies</i>) with few Scots pine (<i>Pinus Sylvestris</i>),\"Oslo municipality, Norway\"
SCION,Coniferous non-native temperate plantation forest consisting of pure radiata pine (<i>Pinus radiata</i>) of 18-20 years of age,New Zealand
ABBY,Managed coniferous forest dominated by Douglas-fir (<i>Pseudotsuga menziesii</i>) with a western hemlock (<i>Tsuga heterophylla</i>) component,\"Clark County, Washington (USA)\"
DEJU,\"Scrubby evergreen forest, where stunted trees like black spruce (<i>Picea mariana</i>) and white spruce (<i>Picea glauca</i>) dominate the canopy\",central Alaska (USA)
JERC,Humid subtropical forest dominated by longleaf pine (<i>Pinus palustris</i>),\"Baker County, Georgia (USA)\"
MOAB,\"Arid, high desert where Pinyon Pine (<i>Pinus edulis</i>) and Utah Juniper (<i>Juniperus osteosperma</i>) make up the canopy\",\"San Juan County, Utah (USA)\"
NIWO,Coniferous subalpine forest with Engelmann spruce (<i>Picea engelmannii</i>) and subalpine fir (<i>Abies lasiocarpa</i>) at higher elevevations and lodgepole pine (<i>Pinus contorta</i>) at lower elevations,\"Boulder County, Colorado (USA)\"
OSBS,Humid subtropical forest dominated by longleaf pine (<i>Pinus palustris</i>),\"Putnam County, Florida (USA)\"
TALL,Humid subtropical forest with canopy dominated by longleaf pine (<i>Pinus palustris</i>) and loblolly pine (<i>Pinus taeda</i>),\"Bibb County, Alabama (USA)\"
")

# first aggregate the fi stats which are at the plot level
lidar_sum_fi %>% 
  dplyr::select(-plotID) %>% 
  dplyr::group_by(siteID) %>% 
  dplyr::summarise(
  ...
  dplyr::mutate(src = paste0("FOR-instance", "<br>", "Puliti et al. (2023)")) %>% 
  dplyr::bind_rows(
    lidar_sum_neon %>% 
      dplyr::mutate(src = paste0("NEON", "<br>", "Weinstein et al. (2021)"))
  ) %>% 
  # add count of training data
  dplyr::left_join(
    lidar_df %>% 
      sf::st_drop_geometry() %>% 
      dplyr::filter(
  ...
  dplyr::mutate(n_t = dplyr::coalesce(n_t.x,n_t.y,0)) %>% 
  # add description
  dplyr::left_join(
    df_temp
    , by = "siteID"
  ...
  dplyr::select(
    ...
    , desc
    , n_t, n
    , area_m2_mean, points_per_m2_mean
    , location
  ) %>% 
  dplyr::mutate(
    dplyr::across(c(n,area_m2_mean), ~ scales::comma(.x,accuracy=1))
    , points_per_m2_mean = scales::comma(points_per_m2_mean,accuracy=0.1)
  ) %>% 
  kableExtra::kbl(
    caption = "summary of lidar data"
    , digits = 1
    , col.names = c(
      "benchmark","site"
      , "description"
      , "training<br>plots", "evaluation<br>plots"
      ,"mean<br>area m<sup>2</sup>", "mean<br>points m<sup>-2</sup>"
      , "location"
    )
    , escape = F
  ) %>% 
...
conifer_spp <-
  ...
  dplyr::distinct(taxonID, vernacularName, genus, species)
...
NeonTreeEvaluation::field %>% 
  ...
  dplyr::filter(is_conifer == 1) %>% 
  dplyr::count(siteID, taxonID, vernacularName, genus, species) %>% 
  dplyr::ungroup() %>% 
  dplyr::arrange(siteID, desc(n)) %>% 
  View()
```
