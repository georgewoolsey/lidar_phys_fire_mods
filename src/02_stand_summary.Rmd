...
let's make a pretty image for use in the journal article. we'll use the `ggmap` package to get a nice background map and do some transformation to actually work with the map.
...
#########################################################################
  ...
  # plot
  plt_basemap <- ggmap::ggmap(hey_ggmap_aligned) + 
    ggplot2::coord_sf(
      expand = FALSE
    ) +
    ggplot2::theme_light() +
...
remove(buffer_box, hey_ggmap, hey_ggmap_aligned, zoom_level, zoom_level)
gc()
```
...
write_temp <- crowns_sf$unit_id %>%
  unique() %>%
  purrr::map(\(x) write_fn(x=x, data = crowns_sf, fprefix = "final_detected"))
```
