  ...
  plt_basemap + 
      ...
      ggplot2::geom_sf(
        data = stand_sf %>% 
          ...
          sf::st_transform(crs=plt_crs)
        , fill = NA, color = my_pal[2], lwd = 1.4
        , inherit.aes = F
      )
plt2_temp
```
...
ggplot2::ggsave(
  "../data/mogollin_rim_study_area_unit2.jpg"
  , dpi = "print"
  , height = 7, width = 7
)
```

plot it with the first unit
...
plt1_temp <- 
  plt_basemap + 
      ...
      ggplot2::geom_sf(
        data = stand_sf %>% 
          dplyr::filter(unit_id==1) %>% 
          sf::st_transform(crs=plt_crs)
        , fill = NA, color = my_pal[1], lwd = 1.4
        , inherit.aes = F
      )
plt1_temp
```
...
```{r, include=FALSE, eval=TRUE}
ggplot2::ggsave(
  "../data/mogollin_rim_study_area_unit1.jpg"
  , dpi = "print"
  , height = 7, width = 7
)
```
...
```{r, include=FALSE, eval=TRUE}
ggplot2::ggsave(
  "../data/mogollin_rim_study_area_inset_unit2.jpg"
  , dpi = "print"
  , height = 8, width = 8
)
```
...
lab_list_temp <- treetops_sf %>% 
  ...
  dplyr::mutate(
    forest_type_group = stringr::str_remove(forest_type_group, " group") %>% 
...
plot_raster_stand_fn(
  ...
  , lookup_code = "forest_type_group_code"
  , lookup_lab = "forest_type_group"
)
```
...
```{r, include=F,eval=TRUE,results='hide',message=FALSE}
# save images to align in powerpoint :\
# remove
list.files(
  "../data/"
  , pattern = "raster_panel_.*\\jpg$"
  , recursive = T, full.names = T
) %>% 
purrr::map(file.remove)

# the forest type plt is special
plt_list[[1]][[2]] +
      ggplot2::theme(
        # legend
        legend.position = c(-0.2, 0.97) # Place legend at top-left
        , legend.justification = c("left", "top") # c(0, 1) # Justify content to top-left
        , legend.box.margin = ggplot2::margin(0, 0, 0, 0, unit = "mm") # Add margin around the legend box
        , legend.key.height = unit(1.3, "cm")
        , legend.key.width = unit(1, "cm")
        , legend.text = ggplot2::element_text(size=17.5)
        , legend.title = ggplot2::element_text(size=17.5)
        # plot
        , plot.subtitle = ggplot2::element_blank()
        , plot.title = ggplot2::element_blank()
        , plot.margin = ggplot2::margin(0, 2, 2, 10, unit = "mm")
      ) +
      # ggplot2::guides(
      #   fill = ggplot2::guide_colorbar(barheight = unit(10, "cm"))
      # )
      ggspatial::annotation_scale(
        location = "bl"
        , style = "bar" #"ticks"
...
plt_temp <- c(2) %>% 
  purrr::map(function(x,my_stand_sf=stand_sf,my_plt_list=plt_list){
    ...
    patchwork::wrap_plots(plts) +
      patchwork::plot_annotation(
        theme = ggplot2::theme(plot.title = ggplot2::element_blank())
      ) &
      ggplot2::theme(
        plot.subtitle = ggplot2::element_blank()
        , legend.text = ggplot2::element_text(size=6.5)
...
use_bbox_temp <- T # use a bbox of the stand polygon or not?
buffer_temp <- 100 # buffer either the polygon or the bbox by this much
...
# crowns_sf %>% nrow()
# crowns_sf %>% sf::st_drop_geometry() %>% dplyr::distinct(treeID) %>% nrow()
# crowns_sf %>% sf::st_drop_geometry() %>% dplyr::distinct(treeID, unit_id) %>% nrow()
```
...
# write fn
write_fn <- function(x, data, fprefix = "final_detected", new_crs = proj_crs, my_outdir = outdir){
    # dsn's
    cf <- file.path( my_outdir, paste0(fprefix, "_crowns_unit_",x,".gpkg") )
    tf <- file.path( my_outdir, paste0(fprefix, "_tree_tops_unit_",x,".gpkg") )
...
write_temp <- crowns_sf$unit_id %>%
  unique() %>%
  purrr::map(\(x) write_fn(x=x, data = crowns_sf, fprefix = paste0("buffered_epsg",5070,"_final_detected"), new_crs = 5070))
```
...
treetops_sf %>% 
  ...
  ggplot() + 
  geom_sf(col = "blue") +
  geom_sf(data = stand_sf %>% dplyr::filter(unit_id==1), col = "red")
```
...
# write crowns and tree tops
write_temp <- simp_crowns_sf$unit_id %>%
  unique() %>%
  purrr::map(\(x) write_fn(x=x, data = simp_crowns_sf, fprefix = "simplified"))
