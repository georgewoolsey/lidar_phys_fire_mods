...
output_temp <- lidR::catalog_apply(
  ...
  , poly_df = simplify_multipolygon_crowns(cloud2trees_ans$crowns_sf)
  , force_crs = T
)
...
`lidR::catalog_apply()` wrote a single .txt file...let's open up this present

```{r}
hmd_df2 <- readr::read_delim(output_temp, progress = F, show_col_types = F)
# check it
...
hmd_df2 <- cloud2trees_ans$crowns_sf %>% 
  dplyr::left_join(
    hmd_df2 %>% 
      dplyr::mutate(
        max_crown_diam_height_m = ifelse(n_pts<3, as.numeric(NA), max_crown_diam_height_m)
      ) %>% 
      dplyr::select(-n_pts)
    , by = "treeID"
  )
...
trees_hmd <- function(
  ...
  if(
    ...
    stop(paste0(
      "`trees_poly` data must contain `treeID` column to estimate missing HMD values."
      , "\nProvide the `treeID` as a unique identifier of individual trees."
    ))
  }else{
  ...
  # # build return data
  ##################################
  if(stringr::str_ends(output_temp, ".*\\.(txt|csv)$")){
    # read the output file
    hmd_df <- readr::read_delim(output_temp, progress = F, show_col_types = F)
