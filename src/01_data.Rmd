...
plot(xxx, color = "Z", legend = T, bg = "white")

xxx@data %>% dplyr::count(treeID)
xxx@data %>% dplyr::count(Classification)
...
see this section of the [package README](https://github.com/olgaviedma/LadderFuelsR?tab=readme-ov-file#9gaps-and-fuel-layers-base-height-fbh)

uses the LAD profiles derived from normalized point cloud data after applying 
`leafR::lad.profile()` function and passes this data to the `LadderFuelsR::get_gaps_fbhs()` function to:
...
it also requires a treeID column, let's update our data frame with all of the `leafR` results
...
gaps_fbhs <- 
  ...
  # LadderFuelsR::get_gaps_fbhs(
    LAD_profiles = leafr_df %>% 
      ...
      # this function HATES extra columns
...
this is a weird data and the `gap` and `cbh` columns are character
...
what are these used for? plot gaps and fuel layers base height (fbh)?
...
leafr_df %>% 
  dplyr::filter(tree_index == ti) %>% 
ggplot() + 
  geom_path(mapping = aes(x = lad, y = height)) +
  geom_point(mapping = aes(x = lad, y = height)) +
  # gaps data
  geom_hline(
    data = gaps_fbhs %>% 
      dplyr::select(
...
### This is so convoluted...let's just cut the superfluity

cut the superfluity (i'll see your  "depurating" and raise you "superfluity")

the [package README](https://github.com/olgaviedma/LadderFuelsR) is sooo difficult to follow. someone had to identify the minimum steps needed to get CBH and cut out the superfluous clutter (bonus points if you see what i did there ;).
...
lad_profile <- lad_profile %>% dplyr::mutate(tree_index = ti, treeID = factor(tree_index))
## LadderFuelsR::get_gaps_fbhs
(
gaps_fbhs <- 
  gw_get_gaps_fbhs(
...
real_fbh <- LadderFuelsR::get_real_fbh(depth_metrics = metrics_depth, step= 1, number_steps = 1, min_height=1.5)
