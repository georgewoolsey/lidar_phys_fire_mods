...
# c(w,h) %>% `*`(.001137) %>% round() %>% paste(collapse = ",") # this returns the w,h to enter for size
# max_area <- c(w,h) %>% `*`(.001137) %>% round() %>% prod() 
# # chunk this
# # x combinations
#   x_l <- dplyr::tibble(
#       xmin = seq(from = map_ext[1], to = map_ext[3], length.out = 20)
#     ) %>% 
#     dplyr::mutate(xmax = dplyr::lead(xmin, n = 1)) %>% 
#     dplyr::filter(!is.na(xmax))
# # y combinations
#   y_l <- dplyr::tibble(
#       ymin = seq(from = map_ext[2], to = map_ext[4], length.out = 20)
#     ) %>% 
...
#   dplyr::filter(dplyr::row_number() == 88) %>% 
#   c() %>% unlist() %>% paste(collapse = ",")
...
#### !!!!! this dataset is not going to work because i couldn't figure out how to get the .tiff from
#### !!!!! ...the image server at 30x30 resolution...the best i could do using the parameters below was 880x880 ;\
...
# huh?
foresttype_lookup %>% dplyr::glimpse()
foresttype_lookup %>% View()
```

let's plot the Forest Type Groups dataset for our example area using our fancy lookup data
