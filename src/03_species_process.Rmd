...
### Get full coverage forest type

now we need to figure out what to do if the tree data overlaps with raster cells with no forest type group...an idea i just had is to create a raster dataset with full coverage of forest type groups and just fill non-forest cells with the value from the nearest forest type. this "full coverage" data set could then be the master data we upload/download from Zenodo (since we're already using custom data) and apply the `terra::extract()` function


first, mark all non-forest cells as "NA" in the raster
...
```{r}
# total cells
terra::ncell(foresttype_fc)
# na cells
terra::global(foresttype_fc, fun = "isNA")
```

here is a toy example of what we're going to do

```{r}
# toy example
r_temp <- terra::rast(ncols=100, nrows=100)
r1_temp <- terra::setValues(r_temp, runif(ncell(r_temp)))
r1_temp[500:2000] <- NA
r1_temp
terra::plot(r1_temp)
# fill
y1_temp <- terra::focal(r1_temp, w = 3, fun = "modal", na.policy = "only", na.rm = T)
terra::plot(y1_temp, main = "first na fill")
# fill again
y1_temp <- terra::focal(y1_temp, w = 3, fun = "modal", na.policy = "only", na.rm = T)
terra::plot(y1_temp, main = "second na fill")
# ok, function to keep filling until no na
fill_rast_na <- function(rast, my_fun = "modal") {
  # count na's
  na_n <- terra::global(rast, fun = "isNA")
  while(na_n>0){
    rast <- terra::focal(rast, w = 3, fun = my_fun, na.policy = "only", na.rm = T)
    # counter
    na_n <- terra::global(rast, fun = "isNA")
  }
  return(rast)
}
# apply the full fill function
y1_temp <- fill_rast_na(rast = y1_temp)
terra::plot(y1_temp, main = "final na fill")
```

original plot of the forest type raster

```{r}
foresttype_fc %>% terra::plot(main = "original forest type raster")
```

let's take the `fill_rast_na()` for a spin with our forest type raster...it might take a while with so many empty cells

```{r}
foresttype_fc <- fill_rast_na(foresttype_fc)
```

.....
..................
........................
...and a few hours later we are back

with the updated data plot

```{r}
foresttype_fc %>% terra::plot(main = "NA filled forest type raster")
```

let's save this for upload to Zenodo

```{r}
terra::writeRaster(
  foresttype_fc
  , filename = file.path(savedir_temp, "foresttype_full_cov.tif")
  , overwrite = T
)
```
