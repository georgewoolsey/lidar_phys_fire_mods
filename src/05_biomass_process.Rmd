...
lf_values_recode <- ifelse(lf_levels[[1]]$Value<=0,NA,lf_levels[[1]]$Value) / 100
# matrix
...
```{r}
calc_rast_cell_overlap(rast = landfire_cbd_recode, poly = las_ctg_sf) %>% 
  purrr::pluck("df") %>% 
  dplyr::glimpse()
```
...
!!!!!!!!!!!!!!!!!!! need to make this function more general to aggregate to carry (or not carry) the landfire CBD value and the fia forest type group 
...
Let's see if we can get estimates that align with the biomass of only the crown portion of the tree

### `allodb::new_equations()` for crown biomass
...
What equations are represented if we filter the list in our attempt to get the biomass of only the crown portion of the tree?
