...
# what?
tree_list_temp %>% dplyr::select(-dplyr::any_of(col_not_sel)) %>% dplyr::glimpse()
```

now we can attach our stand data to the tree list and calculate tree biomass in kg using our estimate of individual tree volume
...
tree_list_temp <- 
  ...
  dplyr::mutate(cruz_biomass_kg = kg_per_m3*crown_volume_m3)
```
...
"https://landfire.gov/data-downloads/US_240/LF2023_FBFM13_240_CONUS.zip"
```
